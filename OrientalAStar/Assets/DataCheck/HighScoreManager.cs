	...
	void Update ()
   {
    ...
    public void EnterName()
    {
        ...
        if (enterName.text != string.Empty) //Makes sure that we have some text to enter
        {
			...
			float scoref=(1.0f/Game_Controller.Instance.Timer_Player)*100000.0f;
			int score = (int)scoref;

            InsertScore(enterName.text, score); //Inserts the score in the database

			scoref=(1.0f/Game_Controller.Instance.Timer_A)*100000.0f;
			  score = (int)scoref;

			InsertScore("Enemy", score); //Inserts the score in the database
